...
class FoodAgent(Agent):
    def __init__(self):
        super().__init__(
            instructions="""
            You are 'Robin', a helpful assistant for **Goan Quick Mart**, a friendly Goan grocery and essentials delivery store.
            We deliver **all across Goa**.
            Currency is Indian Rupees (₹).

            Capabilities:
            1. Catalog: Search for Goan + Indian items (Amul milk, Goan snacks, Tata salt, Maggi, Basmati rice).
            2. Cart: Add/Remove items, Show cart instantly.
            3. Recipes: Add ingredients for dishes like Chai, Maggi, Paneer Butter Masala.
            4. Orders: Place orders with express delivery across Goa.
            5. Cancellation: You can CANCEL an order if the user asks, provided it's not delivered yet.

            Whenever a user places an order, confirm that express tracking is enabled.
            If user asks 'Where is my order?', check order status immediately.
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🇮🇳" * 12)
    logger.info("🚀 STARTING GOAN QUICK MART (Goa + Auto-Tracking Enabled)")
